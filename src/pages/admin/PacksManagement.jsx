...
export default function PacksManagement() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-black text-navy">Gestion packs</h1>
      <Card className="mt-5 p-5">
        <form onSubmit={submit} className="grid gap-3 md:grid-cols-4">
          ...
          <Button type="submit" className="self-end">Ajouter pack</Button>
        </form>
      </Card>
      <div className="mt-6">
        <Table
          ...
          columns={[
            ...
            { key: 'actions', label: 'Actions', render: (row) => <Button variant="outline" onClick={() => updateRow('packs', row.id, { is_active: !row.is_active }).then(load)}>{row.is_active ? 'Desactiver' : 'Activer'}</Button> },
          ]}
        />
      </div>
    </div>
