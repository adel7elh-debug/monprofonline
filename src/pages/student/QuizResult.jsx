...
export default function QuizResult() {
  ...
  if (!state) return <AlertMessage type="warning">Aucun resultat a afficher.</AlertMessage>;
  const { questions, selected, attempt } = state;
  return (
    <div className="mx-auto max-w-4xl">
      <Card className="p-6">
        <h1 className="text-3xl font-black text-navy">Resultat final</h1>
        ...
        {questions.map((question) => {
          ...
          return (
            <Card key={question.id} className="p-5">
              ...
              <p className={`mt-2 text-sm font-bold ${ok ? 'text-emerald-700' : 'text-red-700'}`}>{ok ? 'Bonne reponse' : 'Mauvaise reponse'}</p>
      ...
      <div className="mt-6 flex gap-2">
        ...
        <Button variant="outline" onClick={() => navigate('/student/quizzes')}>Retour QCM</Button>
      </div>
    </div>
  );
}
